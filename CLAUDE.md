...
- `src/game/loot.py`: `open_lootbox` rolls coins/item from a lootbox rarity; `break_crate` rolls the smaller coins/common-item reward from a smashed shop or tavern crate (coins credited straight away, the item left for the caller to drop on the floor); `open_poi_cache` is the better-odds, properly rarity-rolled version for a wilderness ruins/camp POI; all three draw their item from `_roll_loot_item` (weapon/armor/accessory/ammo/potion, one shared name table)
...
- `src/ui/game_renderer.py`: `GameRenderer`, draws the world, entities, camera-relative UI; `_draw_potion_bar` is the potion quickbar above the health bar (one slot per number key) with `_draw_buff_chips` showing the live buffs and their countdown over it
...
- `src/ui/menus/inventory_menu.py`: `InventoryMenu(BaseMenu)`, item list, equip/unequip, clicking a potion drinks it
- `src/ui/menus/shop_menu.py`: `ShopMenu(BaseMenu)`, `_sell_price`, buy/sell UI and pricing (bartering stat and NPC affinity both swing prices); a stack (ammo, potions) sells one unit per click
