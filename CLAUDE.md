...
- `src/game/world.py`: `World`, the state everything reads (entity lists, buildings, shops, saving) plus the per-frame `update`; six jobs are mixed in from `combat.py`, `projectiles.py`, `streaming.py`, `places.py`, `navigation.py` and `villagers.py`. Also here: chunk-bucketed building lookup, `blocked` and what is solid where, spawn caps and safe placement (`ring_search` is the one outward search every placement goes through), terrain speed, impulses and `unstick`, and the two lookups a body's whereabouts is asked through besides `building_at` (`doorway_building_at`, `bed_taken`)
...
- `World` is one class split across six files by mixin (`world.py` state, `combat.py` blows, `streaming.py` the map, `places.py` what happens at a place, `navigation.py` getting there, `villagers.py` what a villager does with their frame). They share the same entity lists; pick the file by what you are changing, not by defaulting to `world.py`.
