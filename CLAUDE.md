...
- `src/game/entities/npcs.py`: `NPC(Entity)`, tracks per-NPC `affinity` (relationship level moved by concrete player actions, quest completion and shop trades, feeds dialogue tone/quest rewards/shop prices)
...
- `src/game/entities/critter.py`: `Critter`, `pick_critter_kind`, non-hostile ambient wildlife (rabbit/deer/fox) that wanders freely and skitters away if the player gets close; no hp, can't be fought, session-only like particles or projectiles (never saved), spawned/despawned around the player by `World` the same way monsters are but without a combat cap
