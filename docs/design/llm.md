...
player is waiting on, and the decisions read off it) go ahead of background work, so quest analysis queued as one conversation
...
opening a conversation on top of work already in flight; a decision is a fraction of a second
and never counts (`DECISION_PREFIX`).

Where the game needs a choice rather than text, it asks for a decision instead
(`decisions.md`): the probabilities of a few lettered answers, read off one pass over the prompt.
...
last finished sentence, the quest parser repairs the model's near-JSON (whether there is a quest
