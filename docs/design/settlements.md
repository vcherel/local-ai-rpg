...
- A settlement's lanes and the roads outside it are one network: `plan_streets` routes every lane round the building footprints off one flood fill from the plaza, and lays one out to each point a road from a neighbour stops at (`terrain.road_ends_at`). A lane is never laid over a footprint, and never out of a gate no road came to. Two lanes running side by side are one lane: a route that comes within `STREET_MERGE_CELLS` of one already worn joins it and is its cells from there, the gateways being worn first and never diverted since a lane out of a gate is the road outside carrying on. A lane is the corners it turns and the width it has at each, drawn as one surface in two passes (edge, then earth), and how wide a stretch is, is how many doors walk it (`lane_width`: a spur to one door against the trunk they all share, meeting it at the trunk's own width). The earth runs to the door leaf itself. The one out of a gate is a trunk however few use it, starts at the arriving road's width and narrows on the way in, and its colour and its verge follow that width (`Village._lane_look`); it laps a width up the road along the road's own heading and lays no verge of its own there, since a cap drawn on ground the road has already verged is the ring that used to sit at every gate. A lane out of a gate crosses the wall square and dead centre: it is walked back from an anchor `STREET_GATE_RUN` inside the gateway with the road end and the gate's own middle laid in front of it (`Village._gate_spine`), and no other lane may be routed through the corners of a gap (`StreetGrid._jambs`), so the dodging round the first house behind the gate happens inside the town rather than across the leaves. Where a stretch meets a busier one it opens into it over `STREET_JOIN` of ground (`widen_into`), a branch worn into a street rather than a step across it; two roads that stop at the same place are one lane out of that gate, not the same lane worn twice.
...
- A village keeps hours, and rings a bell to say so (`WorldVillagers._keep_curfew`, heard only from inside the settlement, so a street emptying itself is something the player watched start). Past `DayNight.CURFEW_DARKNESS` everyone not fighting goes home and gets into their own bed (`World._npc_sleeps`, one bed per person by `_bed_for`, the door shut by the last one in), each on their own hour rolled off their home (`NPC.bedtime_delay_ms`, later again over the tavern) so the street thins out over the minute after the bell rather than emptying on one frame (and nobody stops to stare at the player while waiting on their hour), walked home at `Villages.HOME_WALK_PACE` of the running speed rather than run, and each of them carries a lamp for exactly the length of that walk (`GameRenderer.draw_lanterns`), which is what a settlement's night looks like from a hill above it. The gates lean shut, along with every door in the place but the tavern's: that one the hour never bars, and a walled settlement puts a guard on it and charges for the room (`_post_doorman`, `World.room_price`), a room paid for being the one bed in the game nobody is a witness to. A household is never dealt more people than the room was furnished with beds for (`World._populate_npcs`), only its sleepers are dealt one (a guard's night is their post), and a house always fits at least one bed, the rug giving way if it must; whoever the house still has no bed for lies down on its floor (`_bed_down`) rather than standing in the room until dawn, and either way they are drawn asleep rather than stopped: covers up to the chest, a bedroll under the one on the floor, z's over both. A sleeper is the one body put on top of a solid on purpose, so it is the one exempt from `unstick` and `unwedge` besides the tower archer, and anything to do at all puts them on their feet first. A bed with somebody in it is not a bed the player can take (`World.bed_taken`). Shutting for the night is not barring: a shut gate opens to one press from either side (`Village.push_open`) and answers no weapon; only a grudge or a real mob puts the beam across.
- A watch is walked, not stood. A guard holds a post rather than a street (`Villages.GUARD_POST_RADIUS`), but they cover it at a stroll and barely stop on it, and while they are stopped their head sweeps across it (`NPC._keep_watch`, off their own post so two on a gate are never in step). It is only ever the facing: what a cone is for is telling the player which way somebody is looking, so a guard whose head moves is a guard whose cone can be waited out.
...
- A leaf standing open is never barred: the settlement puts the beam across on the frame the hour or its temper changes and shuts what was open then (`WorldVillagers._bar_doors`), and a door opened since is a door. Three things bar one and they look the same from the street (`BuildingArt._draw_lock`, the beam across the leaf): the hour and the settlement's temper (`Building.barred_now`, set by `WorldVillagers._bar_doors` at curfew and while the place is hostile) and the house's own roll off its id (`house_locked`). The door never opens for the player from the street while any of them holds: the ways in are the window beside it (a broken pane is a hole in the wall shell, not a decoration) or the leaf itself, put through by the player's own swings off the same `DOOR_HP` a monster bashes down and reported as vandalism like a window; only the roll is thrown off for good, from the inside. A window is aimed at as the pane it is rather than the point at its middle, and nothing reaches the furniture in a room from outside it: a blow only finds what stands on the floor it lands on.
