...
is hostile, `WorldPlaces.amends_at` quotes what it wants (`blood_price`: a base, a step per
...
A deed (`WorldPlaces.record_deed`) belongs to the ground it was done on instead: a point, a
...
`WorldPlaces.raid_village` points a wave at one: `Raid.SIZE_BY_TIER` monsters stood up
...
`WorldPlaces.board_offers` writes the same `{has_quest, quest_type, item_name, ...}` reading
