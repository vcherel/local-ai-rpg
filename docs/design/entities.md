...
*A settlement answers it.* `WorldVillagers.militia_orders` counts bosses among the intruders,
...
- Nothing is ever sealed inside a leaf: whatever stands in a doorway or a gateway is stepped out of it before it shuts, and every mover unsticks each frame, the player included. Anything that means to move and cannot is prised out too (`World.unwedge`), villagers and wildlife alike, since the inside corner of an L is invisible to `blocked` and a body in one passes every test it makes; a tower archer and a sleeper are the only bodies exempt from both, because that is where each of them belongs.
