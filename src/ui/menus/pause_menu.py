...
class PauseMenu(BaseMenu):
    def __init__(self, screen):
        super().__init__(screen, width=360, height=300)
    ...
    def _button_rects(self) -> tuple[pygame.Rect, pygame.Rect, pygame.Rect]:
        ...
        quit_rect = pygame.Rect(x, music.bottom + BUTTON_SPACING, BUTTON_WIDTH, BUTTON_HEIGHT)
        return save, music, quit_rect
    ...
    def handle_event(self, event, on_save=None, on_quit=None) -> bool:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if self.save_button_rect and self.save_button_rect.collidepoint(rel):
                ...
                # The one button here that does not end the pause: hearing what it did is
                # the point, and closing the menu on it would hide the label that changed.
                self._toggle_music()
    ...
    def draw(self):
        ...
        self.save_button_rect, self.music_button_rect, self.quit_button_rect = self._button_rects()
        menu_x, menu_y = self.get_centered_position()
